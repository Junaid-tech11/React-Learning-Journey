...
export default function PostForm({ post }) {
    ...
    const submit = async (data) => {
        if (post) {
            const file = data.image[0] ? appwriteService.uploadFile(
                data.image[0]) : null
    ...
    // This is done via the watch method, which listens to changes in the title field and updates 
    // the slug automatically using the slugTransform function.
    ...
    // When users manually input text into the slug field, the onInput 
    // handler applies the same transformation(slugTransform) and updates 
